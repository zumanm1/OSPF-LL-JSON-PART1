/**
 * Keycloak Integration for NetViz Pro Frontend
...
class KeycloakService {
  ...
  /**
   * Initialize Keycloak with configuration from the backend
   */
  async init(): Promise<boolean> {
    try {
      // NetViz Pro uses gateway proxy at /api
  ...
  private async handleCallback(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('[Keycloak] Token exchange error:', errorData);
  ...
  private setTokens(tokenData: any): void {
    ...
    // Set up refresh timer (refresh at 75% of lifetime)
    ...
    if (stored) {
      try {
        ...
        if (this.tokens && Date.now() < this.tokens.expiresAt) {
          ...
          this.setupRefreshTimer(remainingTime * 0.75);
        } else {
          // Try to refresh
          this.refreshTokens();
        }
