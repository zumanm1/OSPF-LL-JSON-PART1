...
const LinkDetailsPanel: React.FC<LinkDetailsPanelProps> = ({ link, onClose }) => {
    ...
    return (
        <div className="absolute top-4 right-4 w-80 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-right duration-300 z-20">
            {/* Header */}
            <div className="relative h-24 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 flex flex-col items-center justify-center border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-center gap-4 mt-2">
                    <div className="flex flex-col items-center">
                        <div className="w-10 h-10 rounded-full border-2 border-white dark:border-gray-700 flex items-center justify-center text-white font-bold text-xs shadow-lg" style={{ backgroundColor: COUNTRY_COLORS[source.country] }}>
                    ...
                    <div className="flex flex-col items-center">
                        <div className="w-10 h-10 rounded-full border-2 border-white dark:border-gray-700 flex items-center justify-center text-white font-bold text-xs shadow-lg" style={{ backgroundColor: COUNTRY_COLORS[target.country] }}>
            ...
            <div className="px-6 py-6 space-y-6">
                ...
                {/* Status Indicator */}
                <div className={`flex items-center justify-between p-3 rounded-lg border ${isUp ? 'bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800/50' : 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800/50'}`}>
                ...
                <div className="space-y-1">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700/50 overflow-hidden">
                        <div className="flex justify-between items-center p-3 border-b border-gray-200 dark:border-gray-700/50">
                            <div className="flex items-center gap-2">
                                <Router className="w-3 h-3 text-blue-500 dark:text-blue-400" />
                                <span className="text-xs text-gray-700 dark:text-gray-300">{source.id}</span>
                            </div>
                            <span className="text-sm font-mono text-gray-900 dark:text-white">{link.source_interface}</span>
                        </div>
                        <div className="flex justify-between items-center p-3">
                            <div className="flex items-center gap-2">
                                <Router className="w-3 h-3 text-emerald-500 dark:text-emerald-400" />
                                <span className="text-xs text-gray-700 dark:text-gray-300">{target.id}</span>
                            </div>
                            <span className="text-sm font-mono text-gray-900 dark:text-white">{link.target_interface}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-200 dark:border-gray-700/50">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span className="text-lg font-bold text-gray-900 dark:text-white">{link.forward_cost || link.cost}</span>
                            </div>
                        </div>
                        <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-200 dark:border-gray-700/50">
                            <span className="text-[10px] text-gray-500 block mb-1">Reverse Cost</span>
                            <div className="flex items-center gap-1">
                                <ArrowRight className="w-3 h-3 text-gray-400 dark:text-gray-600 rotate-180" />
                                <span className="text-lg font-bold text-gray-900 dark:text-white">{link.reverse_cost || link.cost}</span>
                            </div>
                        </div>
                    </div>
