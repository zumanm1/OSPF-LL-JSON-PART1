...
const CostMatrixModal: React.FC<CostMatrixModalProps> = ({ data, sourceCountry, destCountry, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-gray-900 border border-gray-700 rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-auto flex-1 p-4">
            <table className="w-full border-collapse text-sm">
                <thead>
                    <tr>
                        <th className="p-3 text-left bg-gray-800 text-gray-400 font-medium sticky top-0 left-0 z-20 border-b border-gray-700">
                            Source \ Dest
                        </th>
                        {destNodes.map(node => (
                            <th key={node.id} className="p-3 bg-gray-800 text-gray-200 font-bold sticky top-0 z-10 border-b border-gray-700 min-w-[80px] text-center">
                ...
                <tbody>
                    {matrix.map((row, rowIndex) => (
                        <tr key={sourceNodes[rowIndex].id} className="hover:bg-gray-800/50 transition-colors">
                            <td className="p-3 bg-gray-800 text-gray-200 font-bold sticky left-0 z-10 border-r border-gray-700">
                            ...
                            {row.map((cell, colIndex) => (
                                <td 
                                    ...
                                    className="p-3 text-center border border-gray-800/50"
                                    style={{
